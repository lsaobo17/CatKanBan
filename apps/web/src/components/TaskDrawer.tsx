...
import type { BoardColumn, CreateTaskRequest, Priority, Task } from "../../../../packages/shared/src/index";
...
export function TaskDrawer({ open, task, columns, onClose, onSubmit }: TaskDrawerProps) {
    ...
    >
      <Form form={form} layout="vertical" requiredMark={false}>
        ...
        <Form.Item
          ...
          label="起止日期"
          rules={[
            { required: true, message: "请选择明确起止日期" },
            {
              validator: async (_, value: [Dayjs, Dayjs] | undefined) => {
                if (!value || value.length !== 2) {
                  throw new Error("请选择明确起止日期");
                }
        ...
        <Form.Item name="description" label="描述">
          <Input.TextArea rows={4} placeholder="补充任务背景、交付物或风险" />
        </Form.Item>
      </Form>
    </Drawer>
  );
